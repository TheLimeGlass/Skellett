...
public class Sockets {
	...
	public static Object send(SkellettPacket packet) {
		...
		try {
			...
			Bukkit.getConsoleSender().sendMessage(Skellett.cc(Skellett.prefix + "&cIncorrect SkellettProxy details, No socket found or was denied access. For socket at " + Skellett.spData.getString("Host") + ":" + Skellett.spData.getInt("Port")));
			Skript.error("This error is caused because the port and host are incorrect. If you're 100% sure your details are correct, this can be caused because some server's want a number host rather than localhost. Try inserting 0.0.0.0 or 127.0.0.1 instead of localhost. Error: ");
			e.printStackTrace();
		}
		return answer;
	}
	...
	public static void stop(Boolean restart) {
		...
		if (restart) {
			Bukkit.getConsoleSender().sendMessage(Skellett.cc(Skellett.prefix + "&cBungeecord went offline. Trying to reconnect."));
			run();
		} else {
