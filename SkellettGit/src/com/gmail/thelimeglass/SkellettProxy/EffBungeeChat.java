...
@Syntax("[(skellett[ ][(cord|proxy)]|bungee[ ][cord])] (force|make|execute) [(player|uuid)] %string% [to] (say|chat|command|(run|execute)[ command]) %string% [on [the] bungee[ ][cord]]")
...
public class EffBungeeChat extends Effect {
	...
	public String toString(@Nullable Event paramEvent, boolean paramBoolean) {
		return "[(skellett[ ][(cord|proxy)]|bungee[ ][cord])] (force|make|execute) [(player|uuid)] %string% [to] (say|chat|command|(run|execute)[ command]) %string% [on [the] bungee[ ][cord]]";
	}
	@Override
	protected void execute(Event e) {
		...
		if (uniqueId != null) {
			...
			Sockets.send(new SkellettPacket(false, data, SkellettPacketType.PLAYERCHAT));
		} else {
