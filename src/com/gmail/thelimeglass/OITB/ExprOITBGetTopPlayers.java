...
public class ExprOITBGetTopPlayers extends SimpleExpression<String> {
	
	//[(the|all)] [of] [the] top %integer% players of [the] [OITB] [stat][istic] %StatType%
	...
	public String toString(Event event, boolean b) {
		return "[(the|all)] [of] [the] top %integer% players of [the] [OITB] [stat][istic] %OITBStatEnum%";
	}
	@Override
	protected String[] get(final Event e) {
		...
		for(int i = 0; i < number.getSingle(e); i++){
			topplayers.add(top.get(i).getKey());
		}
